...
- For a create or move with no authorized destination selection, call `calendars.list` once. Exclude only Calendars whose Entity Kind Support is `not_advertised`; retain both `advertised` and `unknown`. Select the sole remaining Calendar by its returned canonical href. If more than one Calendar remains, ask the user to choose before the first write. If none remains, report that no compatible Calendar is available and ask whether the user wants to create one. Use `default` only when its configuration is already established; the schema permitting `default` does not establish one. Resolve an unknown destination with read-only discovery before calling a mutation.
