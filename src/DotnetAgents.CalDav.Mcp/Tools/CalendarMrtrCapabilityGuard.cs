...
/// <c>requestState</c> never leaves the process. A session that declared form elicitation takes that path;
/// any other session keeps the typed <c>unsupported_capability</c> result the tools already return when
...
internal static class CalendarMrtrCapabilityGuard
{
    ...
    /// requires declared form elicitation on the initialize-handshake revisions, where the confirmation
    /// becomes a classic elicitation request.
    /// </summary>
    internal static bool IsConfirmationSupported(
        string? protocolVersion,
        bool mrtrSupported,
        ClientCapabilities? clientCapabilities) =>
        mrtrSupported && (UsesPerRequestCapabilities(protocolVersion) || DeclaresFormElicitation(clientCapabilities));
