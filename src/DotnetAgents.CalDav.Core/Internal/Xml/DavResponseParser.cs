...
internal static class DavResponseParser
{
    ...
    private static CalendarDescriptor? TryParseCalendar(XElement response)
    {
        ...
        return new CalendarDescriptor
        {
            ...
            DavDescription = GetPropValue(response, Dav + "description"),
            Color = CalendarCollectionPropertyValues.ReadColor(GetPropValue(response, AppleCs + "calendar-color")),
            Order = CalendarCollectionPropertyValues.ReadOrder(GetPropValue(response, AppleCs + "calendar-order")),
