...
def incoming():
    for line in sys.stdin.buffer:
        ...
        if 'id' in value:
            ...
            checkpoint=params.get('arguments',{}).get('checkpoint')
            pending[value['id']]=dict(method=value.get('method'),tool=params.get('name'),started=time.time_ns(),
                                      ...
                                      checkpoint_length=len(checkpoint) if isinstance(checkpoint,str) else None,
                                      checkpoint_sha256=hashlib.sha256(checkpoint.encode()).hexdigest() if isinstance(checkpoint,str) else None)
...
for line in child.stdout:
    ...
    if request:
        ...
        checkpoint=result.get('structuredContent',{}).get('checkpoint')
        record(dict(event='response',**request,started_ns=started,elapsed_ms=(time.time_ns()-started)/1e6,
                    returned_checkpoint_length=len(checkpoint) if isinstance(checkpoint,str) else None,
                    returned_checkpoint_sha256=hashlib.sha256(checkpoint.encode()).hexdigest() if isinstance(checkpoint,str) else None,
