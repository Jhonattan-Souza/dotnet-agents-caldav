...
def environment(state, service, otlp=True, exact=False):
    ...
    env.update(CALDAV_URL=state.get('caldav_url',state['url']), CALDAV_USERNAME=state['username'], CALDAV_PASSWORD=state['password'],
               CALDAV_DEFAULT_EVENT_CALENDAR_NAME='RFC fixture events',
               CALDAV_DEFAULT_TODO_CALENDAR_NAME='RFC fixture todos',
               CALDAV_EVALUATION_TIME_ZONE='America/Sao_Paulo',
               OTEL_SERVICE_NAME=service)
    if state.get('profile'):
        env['CALDAV_INTEROPERABILITY_PROFILE'] = state['profile']
    if state.get('calendar_hrefs'):
        env['CALDAV_CALENDAR_HREFS'] = ','.join(state['calendar_hrefs'])
    if otlp:
...
class Client:
    ...
    async def request(self, method, params):
        ...
        params.setdefault('_meta', {
            ...
            'io.modelcontextprotocol/clientInfo': dict(name='caldav-rfc-coverage',version='1.0')})
        self.sequence += 1
...
async def pilot(root, assembly):
    ...
    for tool in ['calendar_entities.query','calendar_occurrences.query','todos.query']:
        async with Client(assembly, environment(state, 'caldav-rfc-pilot-'+tool.split('.')[0])) as client:
