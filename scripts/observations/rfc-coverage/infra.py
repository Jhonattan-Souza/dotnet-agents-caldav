...
def request(state, method, path, body=None, headers=None):
    if urllib.parse.urlparse(state['url']).hostname != '127.0.0.1' or not path.startswith('/'):
        raise ValueError('The fixture driver accepts only its loopback server and absolute paths.')
...
''')
    ...
    for suffix, image, ports, extra in [
        ...
        name = run + '-' + suffix
        args = ['run', '-d', '--name', name, '--label', 'caldav.rfc.owner=' + run,
                '--cpus', '4', '--memory', '2g']
        for port in ports:
            args += ['-p', port]
        args += extra + [image]
    ...
    public = {k: state[k] for k in ['run', 'url', 'dashboard', 'otlp', 'containers', 'profile']}
    public['images'] = {'radicale': RADICALE, 'dashboard': DASHBOARD}
...
def resource(kind, index):
    ...
    lines = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//CalDAV RFC coverage seed 20260905//EN',
             'BEGIN:' + kind, f'UID:rfc-20260905-{kind.lower()}-{index:04d}',
             'DTSTAMP:20260701T000000Z', f'SUMMARY:RFC fixture {kind} {index:04d}',
             'CATEGORIES:RFC,DISPOSABLE']
    if kind == 'VEVENT':
        if index % 20 == 1:
...
def seed(root, count):
    if count < 6:
        raise ValueError('At least six resources per populated calendar are required for pagination coverage.')
    state = json.loads((root / 'infra-private.json').read_text())
    principal = state['home_path']
    if not state.get('principal_exists'):
...
<D:resourcetype><D:collection/><C:calendar/></D:resourcetype><D:displayname>RFC fixture {name}</D:displayname>
...
def verify(state):
    ...
    for name in ['events', 'todos', 'archive']:
        ...
        status, data, _ = request(state, 'PROPFIND', state['home_path'] + name + '/', body, {'Depth':'1'})
        assert status == 207
...
def down(root):
    ...
    for name in reversed(state['containers']):
        if not docker('ps', '-aq', '--filter', 'name=^/' + name + '$'):
            continue
        owner = docker('inspect', '--format', '{{index .Config.Labels "caldav.rfc.owner"}}', name)
        assert owner == state['run']
        docker('rm', '-fv', name)
    path.unlink()
    import shutil
    shutil.rmtree(root / 'radicale-config', ignore_errors=True)
