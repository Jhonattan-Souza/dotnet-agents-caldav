...
run_test_suite_manifest_phase() {
  ...
  local row project trx exact prefix filter environment
  local -a rows
  mapfile -t rows < <(emit_test_suite_manifest_rows "$manifest" "$phase")
  for row in "${rows[@]}"; do
    IFS=$'\x1f' read -r project trx exact prefix filter environment <<< "$row"
    "$callback" "$project" "$trx" "$exact" "$prefix" "$filter" "$environment" </dev/null
  done
}
