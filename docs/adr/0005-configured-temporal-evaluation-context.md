...
This forbids Calendar, CalDAV server, operating-system, process, host, locale, and location inference. It accepts the operational cost of explicit configuration because deterministic interpretation and pre-I/O failure are more valuable than environment-dependent convenience. Occurrence and To-do query-module Starts now use this seam. Every To-do Start resolves caller or configured context before I/O, including a no-window compact Start, so completion/due projection and later continuation pages carry one deterministic frozen authority.
